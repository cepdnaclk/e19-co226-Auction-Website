...
public class RegistrationController {
    ...
    public String resetPassword(@RequestBody PasswordModel passwordModel, HttpServletRequest request) {
        UserInfo user = userService.findUserByEmail(passwordModel.getEmail());
        String url = "";
        if(user!=null) {
    ...
    public String savePassword(@RequestParam("token") String token,
        ...
        Optional<UserInfo> user = userService.getUserByPasswordResetToken(token);
        if(user.isPresent()) {
    ...
    private String passwordResetTokenMail(UserInfo user, String applicationUrl, String token) {
        String url =
                applicationUrl
                        + "/savePassword?token="
                        + token;
    ...
    private void resendVerificationTokenMail(UserInfo user, String applicationUrl, VerificationToken verificationToken) {
        String url =
                applicationUrl
